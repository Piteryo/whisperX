...
class DiarizationPipeline:
    ...
    def __call__(self, audio, min_speakers=None, max_speakers=None, num_speakers=None):
        segments = self.model(audio, min_speakers=min_speakers, max_speakers=max_speakers, num_speakers=num_speakers)
        diarize_df = pd.DataFrame(segments.itertracks(yield_label=True))
        diarize_df["start"] = diarize_df[0].apply(lambda x: x.start)
        diarize_df["end"] = diarize_df[0].apply(lambda x: x.end)
        return diarize_df, segments
    ...
    for seg in result_segments:
        ...
        for wdx, wrow in wdf.iterrows():
            if not np.isnan(wrow["start"]):
                diarize_df["intersection"] = np.minimum(diarize_df["end"], wrow["end"]) - np.maximum(
                    diarize_df["start"], wrow["start"]
                )
                diarize_df["union"] = np.maximum(diarize_df["end"], wrow["end"]) - np.minimum(
                    diarize_df["start"], wrow["start"]
                )
                # remove no hit
                if not fill_nearest:
                ...
                word_seg.append(
                    {
                        ...
                        "end": wrow["end"],
                        "text": f"[{speaker}]: "
                        + seg["text"][int(wrow["segment-text-start"]) : int(wrow["segment-text-end"])],
                    }
                )
